...
reg  [127:0] rdf_dout;
wire rdf_rd_en, af_wr_en, video_valid, PF_interrupt;
...
PixelFeeder DUT (
    ...
    .PF_feedframe(),
    .PF_valid(1'b1), .PF_frame(32'd1), //Use frame# & also "init" DURING reset
    .PF_interrupt(PF_interrupt)
);
...
always @(posedge cpu_clk_g) begin
    if (rst_clkCPU) begin
        ...
        if (PF_interrupt) frame_count <= frame_count + 1;
...
always @(posedge cpu_clk_g) begin
    ...
    if (trigWatch !== trigVal) begin //Frame w/Leading-zeros & upper 5-bits of Y
        $display("INT:%b F#%0d ADDR:%h  F:%b Y:%0d X:%0d",
                    PF_interrupt, frame_count, af_addr_din,
