...
module MIPS150 #(
    ...
    parameter COLT45_REGREAD=0, COLT45_MEMWRITE=1, COLT45_CONTROL=0, COLT45_INSTCMAX=2000 //48
)(
    input clk,
    input rst,
...
*/
    ...
    wire [31: 0] PC_DX;
    wire [31: 0] INST_DX;
    //NOTE: INST_DATA gets changed early by BRAM (no disable availabe) but INST_DX gets held on stall
    ...
    PipelineRegister #( .Width(`BUS_ICTL_width), .Mode(3) )
    ...
    // Instruction fetch selection (Instruction activities are in step with StageWF)
...
assign trace = { // 4 segments of 8 values is 32 values (each 32-bit or 32-bit aligned)
    ...
    MemAddr_M,          MemAddrDX_,         _WDataMasked,
        {16'h1234,
...
//***OFFENDING: MemAddr_M perhaps combined with MemAddr__M 
...
generate if (COLT45_INSTCMAX > 0) begin:_STEPS_
    reg [31:0] DBG_cycle, DBG_step, DBG_instcount;
    initial begin
        $display ("%m");
        $display ("-   -   -   -   -   -   -   -   -   -   -   -");
    end
    ...
    task DO_FINISH; begin
        $display("Ran %d / %d (%d)", DBG_cycle, DBG_step, DBG_instcount);
        $display("");
        regfile.DUMP();
        $stop();
        DBG_on = 1'b0;
    ...
    always@(rst) begin
        ...
        $strobe ("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
        if (rst) begin
            DBG_cycle = 'bz;  DBG_step = 'bz;  DBG_instcount = 'bz;
            if (COLT45_CONTROL) $monitor(" (%d) CTL DX %b", DBG_cycle, IControlDX_);
        end else if (DBG_cycle[0] === 1'bz) begin
            DBG_cycle = 0;  DBG_step = 0;  DBG_instcount = 0;
        end
    end

    always@(posedge clk) if (DBG_cycle >= 0) begin:DBG_RUN_POS
        if (PC_DX[15:0] == 16'h0350) begin
            DBG_on = 1'b1; $display("**BRK: call send() 1");
            $stop();
        end
        if (PC_DX[15:0] == 16'h0360) begin
            DBG_on = 1'b0; $display("**BRK: return send() 1");
            $stop();
        end
        if (PC_DX[15:0] == 16'h0060) begin
            $display("**BRK: entered send()");
            $stop();
        end
        ...
        if (DBG_on) begin
            ...
            $display("%d] RST: %d   STL: %d   STEP: %d    CNT: %d", 
                            DBG_cycle, rst, stall, DBG_step, DBG_instcount);
        end

        DBG_cycle = DBG_cycle + 1;
        if (!stall) begin
            DBG_step = DBG_step + 1;
            if (DBG_on) DBG_instcount = DBG_instcount + 1;
        end
        if (DBG_instcount >= (COLT45_INSTCMAX)) DO_FINISH();
        #1;
...
generate if (COLT45_REGREAD) begin:_REGREAD_ //REG reads are async, but only "care" at clock edge
    always@(posedge clk) if (DBG_on && !rst) begin
...
generate if (COLT45_MEMWRITE) begin:_MEMWRITE_
    always@(posedge clk) if (DBG_on && ~stall && |_WriteMask) begin
