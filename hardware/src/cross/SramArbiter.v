...
reg  [3:0] serv_robin;
reg  [3:0] serv_P, serv_0, serv_1, serv_2, serv_3; //Brief history for write-back (or debug)
...
always @(*) begin
  if (SIMPLE_ROBIN) begin:_SIMPLE_ROBIN_
    serv_next = serv_robin & serv_avail; //Non-SkipAhead robin (extra latency)
  end else begin:_PRIORITY_ROBIN_LUT_
    serv_next = SERV_NONE; //Just cautious, every case already covered below
    case (serv_robin)
      default: //Covers 4'b1000 and catch-all/erroneous
        casex (serv_avail)
          4'b1xxx: serv_next = 4'b1000;
          4'b01xx: serv_next = 4'b0100;
          4'b001x: serv_next = 4'b0010;
          4'b0001: serv_next = 4'b0001;
          default: serv_next = SERV_NONE;
        endcase
      4'b0100:
        casex (serv_avail)
...
reg [(1+(4+18+32))-1:0] drive_thru, drive_delay;
assign {sram_addr_valid, sram_write_mask, sram_addr, sram_data_in} = drive_delay;//drive_thru;

always @(posedge sram_clock) begin
  if (reset) begin
    serv_robin <= SERV_W0;
    {serv_P, serv_0, serv_1, serv_2, serv_3} <= 0;
  end else begin
    if (|serv_next) begin
      serv_robin <= {serv_next[0], serv_next[3:1]};
    end else begin
      serv_robin <= {serv_robin[0], serv_robin[3:1]}; //Could make custom sequencer FSM/Lookup
    end
    {serv_3, serv_2, serv_1, serv_0} <= {serv_2, serv_1, serv_0, serv_P};//serv_next};
    serv_P <= serv_next;
    drive_delay <= drive_thru;
  end
end
...
assign sd0_din = (FAULT_LINES && !sram_data_out_valid) ? BADWORD : sram_data_out,
       sd0_din_en = |(serv_3 & SERV_R0);
assign sd1_din = (FAULT_LINES && !sram_data_out_valid) ? BADWORD : sram_data_out,
       sd1_din_en = |(serv_3 & SERV_R1);
// Indicate faults for obviously mistimed writeback
...
// Translations to RVA style ports (ready=!full, coregen set to assert full during reset)
assign w0_din_ready = !w0_full,     w1_din_ready = !w1_full;
assign r0_din_ready = !r0_full,     r1_din_ready = !r1_full;

// First-word-fallthru makes these like RVA "handshakes" (name signals for debug output)
...
wire sw0_under, sw1_under,  sa0_under,  sa1_under,  sd0_over,   sd1_over;
wire w0_over,   w1_over,    r0_over,    r1_over,    r0_under,   r1_under;
wire fault_any;
//wire [5:0] r0_count,    r1_count;
//wire [5:0] sd0_count,   sd1_count;

// synthesis translate_off
always @(posedge sram_clock) begin
  if (!reset && |serv_avail) begin
    $display("AVAIL:%b ROBIN:%b NEXT:%b  FAULT:%b",// (%0d|%0d)",
        serv_avail, serv_robin, serv_next, fault_any);//, sd0_count, sd1_count);
  end
end
...
SRAM_WRITE_FIFO sw1_fifo(
  ...
  .empty(                  ), //OUT
//Debug/monitor signals
  .overflow(w1_over), .underflow(sw1_under) //OUT
);
...
generate if (FAULT_LINES) begin:_FAULT_LINES_
  (* SHREG_EXTRACT="NO", EQUIVALENT_REGISTER_REMOVAL="OFF", ASYNC_REG="TRUE", OPTIMIZE="OFF" *)
  reg  fault_w0, fault_w1, fault_r0, fault_r1; // Persistent per-clock region
  reg  fault_sdram, fault_extern; // w0|w1|r0|r1 sync'd to sdram-clock
  assign fault_any = fault_sdram | fault_extern | fault_read_miss | fault_read_xtra;
//   This quick gathering seems reasonably reliable:
//     Per-clock faults shouldn't glitch since only ever "set" after initial reset (synchronously).
//     Gathered fault is only "set"...so no need for metastability synchronization.
  always @(posedge sram_clock) begin
    if (reset) begin
      {fault_sdram, fault_extern} <= 0;
    end else begin
      if (sw0_under || sw1_under || sa0_under || sa1_under
          || sd0_over || sd1_over) fault_sdram <= 1;
      if (fault_w0 || fault_w1 || fault_r0 || fault_r1) fault_extern <= 1;
    end
  end
  always @(posedge w0_clock, posedge reset)
    if (reset) fault_w0 <= 0;
    else if (w0_over) fault_w0 <= 1;
  always @(posedge w1_clock, posedge reset)
    if (reset) fault_w1 <= 0;
    else if (w1_over) fault_w1 <= 1;
  always @(posedge r0_clock, posedge reset)
    if (reset) fault_r0 <= 0;
    else if (r0_over || r0_under) fault_r0 <= 1;
  always @(posedge r1_clock, posedge reset)
    if (reset) fault_r1 <= 0;
